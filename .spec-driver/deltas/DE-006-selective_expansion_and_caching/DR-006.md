...
name: Design Revision - Selective expansion and caching
created: '2026-03-10'
updated: '2026-03-11'
status: draft
...
- docs/brief.md
code_impacts:
- path: src/expand/types.ts
  state: new
  description: Expansion type definitions
- path: src/expand/triggers.ts
  state: new
  description: Trigger functions and evaluator
- path: src/expand/merge.ts
  state: new
  description: Raw tree merge (immutable path-cloning)
- path: src/cache/types.ts
  state: new
  description: Cache interface and key types
- path: src/cache/keys.ts
  state: new
  description: Deterministic cache key derivation
- path: src/cache/store.ts
  state: new
  description: File-based cache store
- path: src/cache/index.ts
  state: new
  description: Cache factory and cache-aware fetch helpers
- path: src/config.ts
  state: modified
  description: FetchConfig gains expansion + cache fields with validation
- path: src/orchestrate.ts
  state: modified
  description: Expansion loop + cache-aware fetch wiring
- path: src/cli.ts
  state: modified
  description: New CLI flags for expansion and cache control
verification_alignment:
- id: VT-023
  impact: new
  notes: Cache key generation determinism
- id: VT-024
  impact: new
  notes: Cache store read/write/metadata/atomic writes
- id: VT-025
  impact: new
  notes: Cache hit/miss via fetchNodeCached
- id: VT-026
  impact: new
  notes: Cache invalidation (invalidateFile, clear)
- id: VT-027
  impact: new
  notes: Version-aware cache keys
- id: VT-028
  impact: new
  notes: Cached vs uncached determinism
- id: VT-029
  impact: new
  notes: depthTruncatedContainer trigger
- id: VT-030
  impact: new
  notes: geometryNeeded trigger
- id: VT-031
  impact: new
  notes: evaluateExpansionTriggers dedup/priority/cap
- id: VT-032
  impact: new
  notes: mergeExpansions whole-node replacement + immutability
- id: VT-033
  impact: new
  notes: mergeExpansions soft failure (notFound)
- id: VT-034
  impact: new
  notes: Orchestrate expansion loop end-to-end
- id: VT-035
  impact: new
  notes: Expansion disabled regression boundary
- id: VT-036
  impact: new
  notes: Failed expansion fetch resilience
- id: VT-037
  impact: new
  notes: maxTargets bound enforcement
- id: VT-038
  impact: new
  notes: Config validation constraints
design_decisions:
- DEC-032
- DEC-033
- DEC-034
...
# DR-006 – Selective expansion and caching

## 1. Executive Summary
- **Delta**: [DE-006](./DE-006.md)
- **Status**: draft
- **Last Updated**: 2026-03-11
- **Synopsis**: Add selective child expansion and file-based fetch caching to the pipeline
...
| --- | --- | --- |
| `src/config.ts` | 7-field FetchConfig with resolveConfig | Gains expansion + cache fields with validation constraints |
| `src/orchestrate.ts` | Single-pass fetch → normalize → output | Expansion loop + cache-aware fetch wiring |
| `src/cli.ts` | Basic flags | New flags: --no-expand, --max-expand, --expand-depth, --no-cache, --cache-dir |
...
## 7. Raw Tree Merge (`src/expand/merge.ts`)
...
## 9. Config Changes (`src/config.ts`)
